  ...
  return (
    <div style={{ padding:'20px 18px 100px' }}>
      <div className="fnz-rise" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:16 }}>
        <div>
          <div className="fnz-eyebrow">Movimentações</div>
          <h1 className="fnz-display" style={{ fontSize:36, margin:0, marginTop:4 }}>
            Lança<span className="fnz-display-italic" style={{color:'var(--accent)'}}>mentos</span>
          </h1>
        </div>
      ...
      <div className="fnz-rise" style={{ position:'relative', marginBottom:12 }}>
        <input type="text" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar lançamento…" className="fnz-input" style={{ paddingLeft:40, height:44 }}/>
      ...
      <div className="fnz-rise" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', background:'var(--surface-2)', padding:4, borderRadius:12, marginBottom:12 }}>
        {(['all','income','expense'] as const).map((o,i) => (
          <button key={o} onClick={() => setFilter(o)} style={{ background: filter===o?'var(--surface)':'transparent', border:0, padding:'8px 0', borderRadius:9, fontSize:12, fontWeight:600, color:'var(--ink)', boxShadow: filter===o?'var(--shadow-sm)':'none', transition:'background .15s, box-shadow .15s', cursor:'pointer' }}>
            {['Tudo','Entradas','Saídas'][i]}
          </button>
        ))}
      </div>
      ...
      <div className="fnz-rise" style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:4, marginBottom:14 }} >
        <button onClick={() => setCatFilter(null)} style={{ flexShrink:0, height:28, padding:'0 12px', borderRadius:999, border: catFilter===null?'1.5px solid var(--ink)':'1px solid var(--line-2)', background: catFilter===null?'var(--ink)':'transparent', color: catFilter===null?'var(--bg)':'var(--muted)', fontSize:11, fontWeight:600, cursor:'pointer', transition:'all .15s' }}>
          Todas
        </button>
        {topCats.map(c => (
          <button key={c.id} onClick={() => setCatFilter(catFilter===c.id?null:c.id)} style={{ flexShrink:0, display:'flex', alignItems:'center', gap:4, height:28, padding:'0 10px', borderRadius:999, border: catFilter===c.id?`1.5px solid ${c.color}`:'1px solid var(--line-2)', background: catFilter===c.id?c.color+'18':'transparent', color: catFilter===c.id?c.color:'var(--muted)', fontSize:11, fontWeight:600, cursor:'pointer', transition:'all .15s', whiteSpace:'nowrap' }}>
            {c.emoji} {c.label}
          </button>
        ))}
      </div>
      ...
      {Object.entries(groups).map(([day, txs]) => (
        <div key={day} style={{ marginBottom:16 }}>
          ...
          <div className="fnz-card" style={{ padding:4 }}>
            {txs.map((t, i) => <TxRow key={t.id} tx={t} isLast={i===txs.length-1}/>)}
          </div>
        </div>
      ))}
  ...
  return (
    <div style={{ position:'absolute', inset:0, zIndex:100 }}>
      ...
      <div className="fnz-sheet" style={{ padding:'8px 20px 24px' }}>
        ...
        <div className="fnz-eyebrow" style={{ marginBottom:16 }}>Novo lançamento</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', background:'var(--surface-2)', padding:4, borderRadius:12, marginBottom:16 }}>
          {(['expense','income'] as const).map((k, i) => (
            <button key={k} onClick={() => { setKind(k); setCatId(''); }} style={{ padding:'10px 0', borderRadius:9, border:0, background: kind===k?'var(--surface)':'transparent', fontWeight:600, fontSize:13, color: kind===k?'var(--ink)':'var(--muted)', boxShadow: kind===k?'var(--shadow-sm)':'none', transition:'all .15s', cursor:'pointer' }}>
              {['Despesa','Receita'][i]}
            </button>
          ))}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <input value={desc} onChange={e => { setDesc(e.target.value); setCatId(''); }} placeholder="Descrição (ex: iFood, Salário…)" className="fnz-input"/>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
            {cats.map(c => (
              <button key={c.id} onClick={() => setCatId(c.id === catId ? '' : c.id)} style={{ display:'flex', alignItems:'center', gap:4, height:30, padding:'0 10px', borderRadius:999, border: effectiveCat===c.id?`1.5px solid ${c.color}`:'1px solid var(--line-2)', background: effectiveCat===c.id?c.color+'18':'transparent', fontSize:11, fontWeight:600, color: effectiveCat===c.id?c.color:'var(--muted)', cursor:'pointer', transition:'all .15s', position:'relative' }}>
                {c.emoji} {c.label}
                {c.id === guessed && catId !== c.id && (
                  <span style={{ position:'absolute', top:-4, right:-4, width:8, height:8, background:'var(--accent)', borderRadius:999 }}/>
                )}
              </button>
            ))}
          </div>
        ...
        <button onClick={submit} className="fnz-btn accent" style={{ width:'100%', marginTop:20, height:52 }}>
          <Icon name="check" size={18}/> Salvar lançamento
        </button>
      </div>
    </div>
  );
