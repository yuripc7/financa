...
export function MobileApp() {
  ...
  const [phase, setPhase] = useState<'onboarding'|'auth'|'app'>(
    !state.onboarded ? 'onboarding' : (!state.authed ? 'auth' : 'app')
  );
  ...
  if (phase==='auth') return <AuthScreen onDone={(user:StoredUser)=>{store.set({authed:true,currentUser:user as any,profile:{...state.profile,name:user.name}});setPhase('app');}}/>;
...
export function PhoneFrame({children}:{children:React.ReactNode}){
  const time=useClock();
  return(<div style={{width:'100%',height:'100%',background:'var(--bg)',borderRadius:56,overflow:'hidden',position:'relative',boxShadow:'0 0 0 12px #0a0908,0 30px 80px rgba(0,0,0,0.25)'}}>
...
const DESKTOP_TABS=[{id:'dashboard',label:'Visão Geral',icon:'home'},{id:'entries',label:'Lançamentos',icon:'list'},{id:'reports',label:'Relatório',icon:'chart'},{id:'goals',label:'Metas',icon:'target'},{id:'advisor',label:'Consultor IA',icon:'sparkle'},{id:'settings',label:'Configurações',icon:'settings'}] as const;

export function DesktopApp(){
  const [state,store]=useStore();
  const [tab,setTab]=useState<TabId>('dashboard');
  const [showAddTx,setShowAddTx]=useState(false);
  const [phase,setPhase]=useState<'auth'|'app'>(!state.authed?'auth':'app');
  useEffect(()=>{if(state.theme)document.documentElement.setAttribute('data-theme',state.theme);},[]);
  ...
  if(phase==='auth') return(
    <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)'}}>
      <div style={{width:'100%',maxWidth:420,height:'100%',maxHeight:700,position:'relative',overflow:'hidden',borderRadius:24,boxShadow:'var(--shadow-lg)'}}>
        <AuthScreen onDone={(user:StoredUser)=>{store.set({authed:true,currentUser:user as any,profile:{...state.profile,name:user.name}});setPhase('app');}}/>
      </div>
    </div>
  );
    ...
    <div style={{width:'100%',height:'100%',display:'flex',background:'var(--bg)',color:'var(--ink)',fontFamily:'var(--font-sans)',overflow:'hidden',position:'relative'}}>
      <aside style={{width:240,flexShrink:0,background:'var(--surface)',borderRight:'0.5px solid var(--line)',padding:'28px 16px 24px',display:'flex',flexDirection:'column',gap:4}}>
        ...
        {DESKTOP_TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id as TabId)} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',borderRadius:12,border:0,background:tab===t.id?'var(--surface-2)':'transparent',color:tab===t.id?'var(--ink)':'var(--muted)',fontSize:13,fontWeight:500,textAlign:'left',cursor:'pointer',transition:'background .12s,color .2s'}}>
            <Icon name={t.icon} size={17} stroke={1.6}/><span>{t.label}</span>
            {tab===t.id&&<span style={{marginLeft:'auto',width:4,height:4,background:'var(--accent)',borderRadius:999}}/>}
          </button>
        ))}
        <div style={{flex:1}}/>
        <button onClick={()=>setShowAddTx(true)} className="fnz-btn accent" style={{width:'100%'}}><Icon name="plus" size={16} stroke={2.4}/> Novo lançamento</button>
      ...
      <main style={{flex:1,overflow:'auto',position:'relative'}} className="fnz-scroll">
        <div style={{padding:'20px 32px',borderBottom:'0.5px solid var(--line)',display:'flex',justifyContent:'space-between',alignItems:'center',background:'var(--bg)',position:'sticky',top:0,zIndex:10}}>
          <div><div className="fnz-eyebrow">{summary.label}</div><h1 className="fnz-display" style={{fontSize:24,margin:'2px 0 0'}}>{DESKTOP_TABS.find(t=>t.id===tab)?.label}</h1></div>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <button onClick={()=>{const t=state.theme==='dark'?'light':'dark';store.set({theme:t});document.documentElement.setAttribute('data-theme',t);}} className="fnz-iconbtn"><Icon name={state.theme==='dark'?'sun':'moon'} size={16}/></button>
            <button onClick={()=>store.set({hideBalance:!state.hideBalance})} className="fnz-iconbtn"><Icon name={state.hideBalance?'eye-off':'eye'} size={16}/></button>
          </div>
        </div>
    ...
    <div className="fnz-rise-stagger" style={{display:'grid',gridTemplateColumns:'minmax(0,2fr) minmax(0,1fr)',gap:18}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center',marginTop:14}}>
                <span className="fnz-pill" style={{background:monthDelta>=0?'var(--gain-soft)':'var(--loss-soft)',color:monthDelta>=0?'var(--gain)':'var(--loss)',border:0,fontFamily:'var(--font-mono)',fontWeight:600}}>{monthDelta>=0?'+':''}{deltaPct.toFixed(1)}%</span>
                <span style={{fontSize:12,color:'var(--muted)'}}>vs {lastSummary.label.split(' ')[0]}</span>
              </div>
            </div>
            <StreakChip days={state.profile.streakDays}/>
          </div>
          <div style={{marginTop:20}}><LineChart data={cumulative.map((v,i)=>({value:v,label:i}))} width={580} height={120} color="var(--ink)"/></div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginTop:16}}>
            {[{label:'Entradas',value:summary.income,kind:'income',spark:series.map(d=>d.income)},{label:'Saídas',value:summary.expense,kind:'expense',spark:series.map(d=>d.expense)}].map(k=>(
              <div key={k.label} style={{padding:'14px 16px',borderRadius:14,background:'var(--surface-2)'}}>
                <div className="fnz-eyebrow">{k.label}</div>
                <div className="fnz-display" style={{fontSize:28,marginTop:4}}><span style={{opacity:.4,fontWeight:400}}>R$</span>{(k.value/1000).toFixed(1)}<span style={{fontSize:'.55em',opacity:.5}}>k</span></div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:18}}>
        <div className="fnz-card" style={{padding:20,background:'var(--ink)',color:'var(--bg)',border:0}}>
          ...
          <div className="fnz-display" style={{fontSize:32,marginTop:4,color:fc[fc.length-1].value>=0?'var(--bg)':'#ff6a5b'}}>R$ {Math.abs(fc[fc.length-1].value/1000).toFixed(1)}k</div>
        ...
        <div className="fnz-card" style={{padding:20}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            ...
            <div><div className="fnz-display" style={{fontSize:20}}>Saúde <span className="fnz-display-italic" style={{color:'var(--accent)'}}>boa</span></div><div style={{fontSize:11,color:'var(--muted)',marginTop:4}}>Reserva · gasto fixo · diversif.</div></div>
          </div>
        </div>
      </div>
    </div>
